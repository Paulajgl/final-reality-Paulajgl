...
import effect.{Effect, PoisonEffect}
import enemy.Enemy
        ...
        case whiteMage: WhiteMage if whiteMage.manaPoints >= cost =>
          ...
          weapon match {
            case weapon: MagicalWeapon =>
              poisonEffect.apply(caster, target, weapon)
              caster
          }
