...
/**
 * Represents a paralysis effect that can be applied to a game unit.
 *
 * @param duration The duration of the paralysis effect in turns. Default is 1 turn.
 */
class ParalysisEffect(private var duration: Int = 1) extends Effect {

  /**
   * Applies the paralysis effect to the target game unit.
   *
   * This method is called when the effect is applied by a caster using a magical weapon.
   * If the weapon is equipped, the target's live points are reduced by the weapon's magic attack points.
   * Additionally, the target is marked as paralyzed and a message is printed.
   *
  ...
  override def apply(caster: MagicalCharacter, target: GameUnit, weapon: MagicalWeapon): Unit = {
    if (weapon.isEquipped) {
      target.livePoints = target.livePoints - weapon.magicAttackPoints
      target.isActive == true
      println(s"${target.name} is paralysed.")
    }
  }
   ...
   * is decreased by 1.
   *
   * @param target The game unit affected by the paralysis effect.
   */
